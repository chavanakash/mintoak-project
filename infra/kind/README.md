...
|---|---|
| ECR / container registry | `registry:2` container on `localhost:5001`, wired into the kind Docker network |
...
```
docker build -t localhost:5001/merchant-service:dev backend/merchant-service
docker push localhost:5001/merchant-service:dev
```
